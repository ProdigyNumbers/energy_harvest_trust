...
wget.download(
    ...
    out="/home/ec2-user/s3vol/FireData/rawfiles",
)
# generate date str
today = date.today()
...
os.rename(
    "/home/ec2-user/s3vol/FireData/rawfiles/SUOMI_VIIRS_C2_South_Asia_24h.csv",
    "/home/ec2-user/s3vol/FireData/rawfiles/" + filename,
)
...
data["geometry"] = data.apply(
    lambda x: Point((float(x.LONGITUDE), float(x.LATTITUDE))), axis=1
)
data = gpd.GeoDataFrame(data, geometry="geometry")

data.crs = "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"
data.to_file(
    "/home/ec2-user/s3vol/FireData/outputfiles/"
...
data2.to_csv(
    "/home/ec2-user/s3vol/FireData/outputfiles/SUOMI_VIIRS_C2_South_Asia_24h_"
    + yesterday
    + "_output.csv",
    sep=",",
    date_format="%m-%d-%Y",
...
Ind_frp = Ind_frp.rename(
    columns={
        "State_Total": "State_Pixarea",
        "District_Total": "District_Pixarea",
        "Taluk_Total": "Taluk_Pixarea",
    }
)
...
talu_df = talu_df.rename(
    columns={"NAME_1": "State", "NAME_2": "District", "NAME_3": "Taluk"}, errors="raise"
)
...
ax.axis("off")
ax.set_title(
    f"VIIRS-SNPP Day-Night Fire Detection\nIndia - Taluk Level,{yesterday} UTC",
    fontdict={"fontsize": "25", "fontweight": "3"},
)
sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
a = fig.colorbar(
    sm, orientation="horizontal", extend="both", boundaries=bounds, ticks=vals, pad=0.05
)
...
fig.savefig(
    "/home/ec2-user/s3vol/home/"
    + date_dir
    + "/indiataluk/"
    + "INDIA_TALUK_SNP_VIIRS_FC_"
    + date_dir
    + ".png",
    dpi=150,
)
...
distro = distro.rename(
    columns={"NAME_1": "State", "NAME_2": "District"}, errors="raise"
)
...
ax.axis("off")
ax.set_title(
    f"VIIRS-SNPP Day-Night Fire Detection\nIndia - District Level,{yesterday} UTC",
    fontdict={"fontsize": "25", "fontweight": "3"},
)
sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
...
ax.axis("off")
ax.set_title(
    f"VIIRS-SNPP Day-Night Fire Detection\nIndia - State Level,{yesterday} UTC",
    fontdict={"fontsize": "25", "fontweight": "3"},
)
sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
