...
logger = LoggerFactory("gdrive_download").get_logger()
...
def main():
    ...
    if total != 1:
        logger.info(f"{total} folders found")
    ...
    logger.info(f"{folder_id} {folder_name}")
...
def get_full_path(service, folder):
    ...
    files = (
        service.files()
        .get(fileId=folder["parents"][0], fields="id, name, parents")
        .execute()
    )
    ...
    while "parents" in files:
        files = (
            service.files()
            .get(fileId=files["parents"][0], fields="id, name, parents")
            .execute()
        )
...
def download_folder(service, folder_id, location, folder_name):
    ...
    total = len(result)
    for current, item in enumerate(result, start=1):
        file_id = item["id"]
        filename = item["name"]
